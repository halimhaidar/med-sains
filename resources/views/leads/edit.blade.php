...
@section('content')
    ...
    <div
        ...
        <form class="ps-5" method="GET" action="{{ route('leads.edit', $lead->id) }}">
            <div id="step-1" class="pt-5 ps-5">
                <label for="contact_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                    Contact Name</label>
        ...
        <form action="{{ route('leads.update', $lead->id) }}" method="POST" class="p-10">
            @csrf
            @method('PUT')
            <!-- Step 1: User Details -->
                        ...
                        <div class="col-span-2">
                            ...
                            <select
                                ...
                                @foreach ($user as $item)
                                    <option value="{{ $item->id }}"
                                        {{ isset($lead) && $lead->assign_to == $item->id ? 'selected' : '' }}>
                                        {{ $item->fullname }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @else
                        <div class="col-span-2">
                            <label for="assign_to"
                                ...
                                To</label>
                            <input type="text" 
                                name="assign_to_display" 
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                value="{{ $user->fullname }}" 
                                readonly>
                            <input type="hidden" name="assign_to" value="{{ $user->id }}">
                        </div>
                    @endif
                    <div class="col-span-2">
                        ...
                        <textarea id="description" name="description" rows="4"
                            class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">{{ $lead->description }}</textarea>
                    </div>
                </div>
